...
export interface CardAccount extends AccountBase {
   ...
   * **次回の引落額**（確定済みの請求額。正の値）。
   *
   * 基準日以降で最初に到来する引落日に計上する（CL-2 手順4）。
   * カード会社の明細では「確定金額」として表示されている。
   */
  balance: Yen;
  /**
   * **次回の引落に含まれない未確定の利用額**（正の値）。未入力は0。
   *
   * `balance` の次に到来する引落日に計上する（CL-2 手順4）。
   * カード会社の明細では「未確定分」として表示されている。
   *
   * 締め翌月払いのカードでは、基準日時点で未払の束が2つ同時に存在する。
   * 月末締め・翌月27日払いで基準日が 9/17 なら、8月利用分（締め済み・9/27
   * 引落）と 9/1〜9/16 利用分（未締め・10/27 引落）である。**これを1つに
   * まとめて最初の引落日に載せると、最大1ヶ月ぶんの支出を前倒しで計上する。**
   * 近い将来の残高が実際より低く出て、防衛ラインの警告が誤って鳴る。
   */
  unbilledBalance?: Yen;
  /** 締日（1〜31。31は月末） */
  closingDay: number;
...
export interface Actual extends EntryBase {
  ...
  key: string | null;
  /**
   * `key` が null である理由が「予定に対応しないと利用者が確定した」ことか。
   * 既定は false（＝未判定）。
   *
   * FR-46 の候補提示を抑止するためだけに使い、**CL-1〜CL-6 の計算には
   * 影響しない。** `key` が null であることには「まだ判定していない」と
   * 「判定した結果、突発の支出だった」が混ざっており、後者に候補を出し
   * 続けると却下が機能していないように見える（§3.2、AC-38）。
   */
  unplanned?: boolean;
}
