...
describe("CL-1 期間の絞り込みと整列", () => {
  ...
  /**
   * 【未確定】期間外の回を、オーバーライドで期間内に引き込んだ場合。
   *
   * CL-1 は「対象期間の各月について1件生成する」→「期間外に出たものを除外」
   * としか書いていないため、対象期間の外の月は最初から生成していない。
   * 結果として、7月の回を6月に前倒ししても6月の一覧には現れない。
   *
   * プロトタイプは終了月の翌月まで余分に生成してこの片側だけを拾っていたが、
   * 開始月の前月は拾っておらず対称でない。どちらが仕様かを確認するまで、
   * 仕様の字面どおりの挙動をここに固定しておく。
   */
  it("【未確定】期間外の回を前倒ししても現れない", () => {
    const item = recurring({ id: "r1", day: 15 });
