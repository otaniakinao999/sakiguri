...
export interface EventMap {
  ...
  /**
   * FR-46 の消し込み候補に決着をつけた。
   *
   * **`actual_recorded` を流用しない。** 候補の確定も「予定にない支出」の
   * 確定も、既存の実績を更新する操作であって実績を記録していない。
   * 流用すると実績入力の件数が実際より多く出て、PoC の5指標が汚れる。
   * イベント名が表していない操作で発火させると、そのイベントを数えている
   * 指標が静かに狂う（CLAUDE.md §4.1）。
   *
   * **FR-46 の候補が当たっているかを測る。** FR-46 は途中で追加した機能で
   * 当初の5指標に無いため、これが無いと候補提示が役に立ったのか毎回外れて
   * 邪魔だっただけなのかが分からないまま終わる。CL-7 の条件
   * （同額・±12日・同口座）が緩すぎるかはこれでしか分からない。
   */
  candidate_resolved: {
    /** 候補を確定した（`key` を張った）なら true。却下なら false */
    confirmed: boolean;
    /** そのとき候補が何件あったか。複数が普通なのか稀なのかを知る */
